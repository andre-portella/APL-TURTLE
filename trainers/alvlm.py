...
import os.path as osp
import sys
# Adiciona o diretório pai ao path para que o Python encontre o arquivo
...
class ALVLM(TrainerX):
    ...
    def train(self):
        ...
        # ADICIONADO: permitir que se use com o conjunto de validação (val) como parte do conjunto de treinamento (train)
        #dataset._train_x = list(dataset._train_x) + list(dataset.val)
        ...
        for i in range(8):
            ...
            if self.cfg.TRAINER.COOPAL.METHOD == "cb":
                ...
                if i == 0:
                    ...
                    zsclip.build_model()
                    selector = CB(self.cfg, zsclip, unlabeled_dst, U_index, val_x, dataset.get_num_classes(unlabeled_dst), self.device, i, self.test_loader, dataset)
                else:
